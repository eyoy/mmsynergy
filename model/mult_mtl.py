...
from fusions.mult.mult_mtl import MULTModel 
...
save_path = '/mnt/g/mmsynergy/model/tune/mult_mtl/'
seed = 521
lr_box = [1e-3]
bz_box = [64]
alphas = [0.1, 0.2, 0.3]
betas = [0.6, 0.5, 0.4, 0.3, 0.2, 0.1]
for lr in lr_box:
        for bz in bz_box:
                for a in alphas:
                        for b in betas:
                                ...
                                traindata, validdata,  test_robust = get_dataloader('/mnt/g/mmsynergy/model/data/mtl.pkl', robust_test=False, max_pad=True, batch_size=bz,max_seq_len=40, num_workers=0)
                                ...
                                # lr=lr, clip_val=1, save= save_path+'models/mult_mtl_lr{}_bs{}_alpha{}_beta{}.pt'.format(lr,bz, a, b), weight_decay=0.01, alpha = a, beta = b,
                                # objective=torch.nn.L1Loss(), trainsave= save_path+'checkpoints/train_valid/perf_lr{}_bs{}_alpha{}_beta{}.pt'.format(lr,bz, a, b))
                                print("Testing:")
                                #model = torch.load(save_path+'models/mult_mtl_lr{}_bs{}_alpha{}_beta{}.pt'.format(lr,bz, a, b)).cuda()
                                model = torch.load(save_path+'models/mult_mtl_lr0.001_bs64_alpha0.2_beta0.1.pt').cuda()


                                test(model=model, test_dataloaders_all=test_robust, dataset='mosi', is_packed=False, alpha = 0.5, beta = b,
