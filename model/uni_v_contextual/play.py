...
from training_structures.Supervised_Learning_test_bimodal_vt import test, train  # noqa
...
save_path = '/mnt/g/mmsynergy/model/tune/uni_v_context/play/'

lr_box = [0.005, 0.0005, 0.0001, 0.00001]
bz_box = [16, 32, 64, 128]
hid_dims = [40]
hidhid_dims = [512]
for lr in lr_box:
        ...
        for hid in hid_dims:
            for hidhid in hidhid_dims:
                ...
                traindata, validdata, test_robust = \
                    get_dataloader( '/mnt/g/mmsynergy/model/data/play_contextual_censored.pkl', robust_test=False, batch_size = bz, max_seq_len=50)

                # mosi/mosei
                # encoders = [LSTM(272, 60, dropoutp=0.02).cuda(),
                #             LSTM(53, 400,dropoutp=0.03).cuda()]
                encoders = [Transformer(272, hid).cuda(),
                ...
                train(encoders, fusion, head, traindata, validdata, 150, task="regression", optimtype=torch.optim.AdamW,
