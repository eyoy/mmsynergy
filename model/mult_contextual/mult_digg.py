...
from fusions.mult.mult_modified_cross_contextual import MULTModel  # noqa
...
save_path = '/mnt/g/mmsynergy/model/tune/mult_contextual/digg/'
### The label here all in the comment folder, which shouldbe corrected
seed = 521
lr_box = [1e-3,1e-4,5e-3,5e-4]
bz_box = [16, 64, 128] 
for bz in bz_box:
        for lr in lr_box:
                ...
                traindata, validdata,  test_robust = get_dataloader('/mnt/g/mmsynergy/model/data/digg_contextual_censored.pkl', robust_test=False, max_pad=True, batch_size=bz,max_seq_len=50, num_workers=0)
                ...
                # lr=lr, clip_val=1, save= save_path+'models/comment_lr{}_bs{}.pt'.format(lr,bz), 
                # weight_decay=0.01, objective=torch.nn.L1Loss(), 
                # trainsave= save_path+'checkpoints/train_valid/perf_lr{}_bs{}.pt'.format(lr,bz))
                print("Testing:")
                #model = torch.load(save_path+'models/comment_lr{}_bs{}.pt'.format(lr,bz)).cuda()
                model = torch.load(save_path+'models/comment_lr0.0005_bs1.pt')
